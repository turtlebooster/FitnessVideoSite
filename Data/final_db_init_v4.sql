...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit`.`calendar` ;
...
CREATE TABLE IF NOT EXISTS `ssafit`.`calendar` (
  ...
  `eat_calories` INT NOT NULL DEFAULT 0,
  `use_calories` INT NOT NULL DEFAULT 0,
  PRIMARY KEY (`user_id`, `date`),
  UNIQUE INDEX `todo_id_UNIQUE` (`todo_id` ASC) VISIBLE,
  INDEX `calendar_user_id_idx` (`user_id` ASC) VISIBLE,
  CONSTRAINT `calendar_user_id_FK`
    FOREIGN KEY (`user_id`)
...
CREATE TABLE IF NOT EXISTS `ssafit`.`todo_list` (
  ...
  CONSTRAINT `todo_list_id_FK`
    FOREIGN KEY (`id`)
    REFERENCES `ssafit`.`calendar` (`todo_id`)
    ON DELETE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 3
DEFAULT CHARACTER SET = utf8;
